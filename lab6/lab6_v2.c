...
int thread2startflag = 0;
int FinalOutput[500];
int FinalOutputIndex = 0;
int Thread2HelpingDone = 0;
FILE *fp_thread;
void LED_Blink(int);
//void SevenSeg();
//void KeyboardTouch();
...
pthread_mutex_t file1;
pthread_mutex_t file2;
...
void *CheckForMatch(void *rank)
{
	...
	for (i = 0; i < elementNumber; i++)
	{
		if ( (dequeue[i] % My_MatchNumber) == 0)
		{
			...
			if (my_rank == 1 && outputindex > 5)
			{
				...
				pthread_mutex_lock(&access_dequeue);
				FinalOutput[FinalOutputIndex] = dequeue[i];
				FinalOutputIndex++;
				pthread_mutex_unlock(&access_dequeue);
	...
	if(my_rank == 2)
	{
		...
		fp_thread = fopen(buffer,"w");
		fprintf(fp_thread, "threadID : %d PID : %d \n",my_rank-1,getpid());
		fprintf(fp_thread, "thread %d is writing.....\n",my_rank);
		printf("thread %d is writing.....",my_rank);
		for (i = 0; i < outputindex; i++)
		{
			fprintf(fp_thread, "%d ",OutputFileNumber[i] );
			printf("%d  ",OutputFileNumber[i] );
		}
		printf("\n");
	}
	...
	if(my_rank == 1)
	{
		...
		fprintf(fp_thread, "threadID : %d PID : %d \n",my_rank-1,getpid());
		printf("thread %d is writing.....",my_rank);
		for (i = 0; i < FinalOutputIndex; i++)
		{
			fprintf(fp_thread, "%d ",FinalOutput[i] );
	...
	if( my_rank == 2)
	{
		...
		Thread2HelpingDone = 1;

/*
		while(1)
		{
			if (thread2startflag == 1)
			{
				break;
			}
		}

		sprintf(buffer,"thread%d.txt",my_rank-1);
		pthread_mutex_lock(&access_dequeue);
		fp_thread = fopen(buffer,"w");
		fprintf(fp_thread,"\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
		fprintf(fp_thread,"\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
		...
		printf("thread %d is writing......to \n",my_rank);		
		for (i = 0; i < outputindex; i++)
		{
			fprintf(fp_thread, "thread2 : %d  \n",OutputFileNumber[i] );
			printf("%d  ", OutputFileNumber[i] );
		}
		printf("\n");
		pthread_mutex_unlock(&access_dequeue);
		printf("thread %d unlock",my_rank);
		*/
	}
